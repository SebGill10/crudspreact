...
function App() {
  const baseUrl='https://localhost:44349/api/gestores';
  ...
  return (
    <div className='App'>
        ...
        <tbody>
          {data.map(gestor=>(
            <tr key={gestor.id}>
              ...
              <td>
                <button onClick={()=> abrirCerrarModalInsertar()} className='btn btn-outline-success'>Insertar Gestor</button>
                <button onClick={()=> seleccionarGestor(gestor, 'Editar')} className='btn btn-outline-primary'>Editar</button> {" "}
                <button onClick={()=> seleccionarGestor(gestor, 'Eliminar')} className='btn btn-outline-danger' >Eliminar</button>
              </td>
            </tr>
          ))}
      ...
      <Modal  isOpen = {modalEliminar}>
            <ModalBody>
              ¿Seguro que desea eliminar el registro {gestorSeleccionado && gestorSeleccionado.nombre}?
            </ModalBody>
            <ModalFooter>
